...
import toolbox.postprocessing.param_sweep_custom.lcoe_sweep_tools as lcoe_tools
import pandas as pd
import os
...
def run_lcoe_chunk_of_sites(sitelist,sites_to_run,input_results_dir,output_filepath):
    res = pd.DataFrame()
    for site_id in sites_to_run:
        site_res = lcoe_tools.run_min_lcoe_for_site(input_results_dir,site_id,state = sitelist.loc[site_id]["state"],lat = sitelist.loc[site_id]["latitude"],lon=sitelist.loc["longitude"])
        res = pd.concat([res,site_res],axis=0)
    if not save_pf_config:
        res = res.drop(columns=["lcoe_pf_config"])
...
def main(sitelist,result_dir,output_filepath_base_base):
    ...
    run_lcoe_chunk_of_sites(sitelist,s_list_chunks,result_dir,output_filepath_base_base + f"_{rank}.pkl")
...
if __name__ == "__main__":
    ...
    output_filepath_base_desc = os.path.join(output_results_dir,"2025_min_lcoe_chunks")
