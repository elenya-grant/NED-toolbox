...
import toolbox.finance_reruns.rerun_simplex_for_new_costs as rerun_simplex
import sys
...
def do_something(site_config,site_id):
    ...
    if len(site_files)>0:
        ...
        site_file_desc += "--LCOH_Simplex.pkl"
        # output_filename = os.path.join(new_results_dir,site_file_desc)
        simplex_filename = [f for f in site_files if "--LCOH_Simplex.pkl" in f]
        simplex_filepath = os.path.join(previous_results_dir,simplex_filename[0])
        ...
        rerun_simplex.run_new_costs_for_lcoh_simplex(
            new_results_dir,
            site_config["new_cost_info"]["new_atb_year"],
            site_config["previous_run_info"]["battery_charge_rate_mw"],
            site_config["previous_run_info"]["battery_hours"],
            simplex_filepath,
...
if __name__ == "__main__":
    if len(sys.argv)<2:
        ...
        rerun_input_filename = sys.argv[1]

    input_filepath = INPUT_DIR/"v1-parametricsweep-offgrid-reruns"/rerun_input_filename
